...
### Medium — patterns with invalid POSIX ERE crash at match time
...
1. Validate each pattern at load time (try a benign no-op match against
   an empty string) and skip invalid entries with a warning.
2. Catch regex errors at match time by wrapping the match in a subshell
   or `(set +e; ...)` block and treating an error as "no match".
...
### Medium — `jq` failure aborts the hook hard
...
1. Pre-check `command -v jq` at the top of the script; if missing,
   emit a clear `>&2` warning and `exit 0` (fail-open).
2. Add a `JQ_PATH` env var override so the test harness can inject a
   stub.
...
- Pre-validate ERE patterns at load time.
- Pre-check `jq` availability and fail-open.
- Stricter / more informative section header parsing.
- Optional command normalisation (leading whitespace, `./` prefix).
